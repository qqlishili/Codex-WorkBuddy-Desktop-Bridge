...
class WorkBuddyError(RuntimeError):
    """当 WorkBuddy 桌面桥接器无法完成请求时抛出。"""


def task_prompt(prompt: str, cwd: str) -> str:
    """给 WorkBuddy Task 提供访问上下文，同时保持其原生 cwd 为空。"""
...
def discover_desktop_server() -> DesktopServer:
    """发现正在运行的 WorkBuddy 桌面 ACP host 及其轮换密码。"""
    errors: list[str] = []
    for _, pipe, sidecar_pid in _candidate_sidecars():
        try:
            ...
            # URL 以 OSC-8 终端超链接形式打印。在 BEL 或 ESC 终止符前停止，
            # 不要把终止符当作轮换密码的一部分。
...
def spawn_isolated_server(
    ...
    *,
    ...
    """启动一个 WorkBuddy CLI host，其 ACP 状态对单个任务私有。"""
...
class AcpClient:
    ...
    def configure_session(
        ...
        *,
        ...
        """在首次 prompt 前应用 WorkBuddy session 选项。"""
